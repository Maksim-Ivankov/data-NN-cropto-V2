...
def train_model(X_train, y_train, num_nodes, dropout_prob, lr, batch_size, epochs,activat_1,activat_2,activat_3,potery):
  nn_model = tf.keras.Sequential([
      tf.keras.layers.Dense(dropout_prob, activation=activat_1, input_shape=(10,)),
...
def plot_history(history,title,model,i,val_loss,coin_up,TP,SL):
  print_log(coin_up,i)
  print_log(title,i)
  data_trade = print_trade(model,coin_up,i,TP,SL)
...
def print_trade(model,coin_up,step,TP,SL):
  ...
  bar = progressbar.ProgressBar(maxval=len(df_coin)).start() # прогресс бар в консоли
  trade_mas = []
  scaler = StandardScaler()
  X = scaler.fit_transform(df_coin)
  number_df_test = 0
  predskasanie_celka = []
  for row in X:
    bar.update(number_df_test)
    y = model.predict(np.array([row], dtype=float),verbose = 0)
    predskasanie_celka.append(y[0][0])
    if y[0][0]<0.1:long = 0
  ...
  print_log(f'Минимальное занчане предсказания : {min(predskasanie_celka)}',step)
  print_log(f'Максимальное значение предсказания : {max(predskasanie_celka)}',step)
  print_log('------------------------',step)
  # ниже торговля
...
for i in range(20,30):
  ...
  # coin_up = random.choice(['LDOUSDT', 'LEVERUSDT', 'LINAUSDT', 'LINKUSDT', 'LITUSDT', 'LOOMUSDT', 'LPTUSDT', 'LQTYUSDT', 'LRCUSDT', 'LSKUSDT', 'LTCUSDT', 'MAGICUSDT', 'MANAUSDT', 'MANTAUSDT.csv'])
  ...
  try:
    ...
    plot_history(history,title_graph,model,i,val_loss,coin_up,TP,SL)
